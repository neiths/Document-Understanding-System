...
class MetricsCollector:
        ...
        with self._lock:
            ...
            if duration:
                self.request_duration.labels(
                    pipeline=pipeline, document_type=document_type
                ).observe(duration)

    def record_pipeline_request(
        self, pipeline: str, duration: float, success: bool = True
    ):
    ...
    def record_model_confidence(
        self, pipeline: str, model_name: str, document_type: str, confidence: float
    ):
        """Record model confidence"""
        with self._lock:
            self.model_confidence.labels(
                pipeline=pipeline, model_name=model_name, document_type=document_type
            ).set(confidence)

    def record_model_error(self, pipeline: str, model_name: str, error_type: str):
        """Record model error"""
        with self._lock:
            self.model_errors.labels(
                pipeline=pipeline, model_name=model_name, error_type=error_type
            ).inc()

    def update_gpu_utilization(self, pipeline: str, gpu_id: str, utilization: float):
        """Update GPU utilization"""
        with self._lock:
            self.gpu_utilization.labels(pipeline=pipeline, gpu_id=gpu_id).set(
                utilization
            )
...
class OpenTelemetryTracer:
    ...
    def create_span(self, name: str, func, attributes: Dict[str, Any] = None):
