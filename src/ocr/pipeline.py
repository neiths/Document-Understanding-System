...
class OCRPipeline:
    ...
    async def process(self, request: ExtractionRequest) -> ExtractionResponse:
        ...
        try:
            ...
            # 2. Text Detection
            text_detection_result = await self.text_detector.detect(image_path, layout_result.boxes)
            ...
            confidences = [
                ...
                sum(r.confidence for r in text_recognition_results) / len(text_recognition_results) if text_recognition_results else 0,
            ...
            logger.info(f"OCR processing completed with confidence {overall_confidence:.2f}")
