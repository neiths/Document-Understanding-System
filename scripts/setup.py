...
def setup_mlflow():
    ...
    run_command(
        "docker run -d -p 5000:5000 -e AWS_ACCESS_KEY_ID=minioadmin -e AWS_SECRET_ACCESS_KEY=minioadmin -e MLFLOW_S3_ENDPOINT_URL=http://localhost:9000 --name mlflow mlflow/mlflow:latest"
    )
...
def setup_minio():
    ...
    run_command(
        "docker run -d -p 9000:9000 -p 9001:9001 -e MINIO_ROOT_USER=minioadmin -e MINIO_ROOT_PASSWORD=minioadmin -v minio_data:/data --name minio minio/minio:latest server /data --console-address :9001"
    )
...
def main():
    ...
    parser.add_argument(
        "--download-models", action="store_true", help="Download pretrained models"
    )
    parser.add_argument(
        "--setup-infrastructure", action="store_true", help="Setup MLflow and MinIO"
    )
    parser.add_argument("--start", action="store_true", help="Start the entire system")
    parser.add_argument("--test", action="store_true", help="Run tests")
    parser.add_argument(
        "--full-setup", action="store_true", help="Run full setup process"
    )
    ...
    if args.check_requirements:
        ...
        if not gpu_available:
            print(
                "⚠️  GPU not detected. The system will run on CPU, but performance will be slower."
            )
    ...
    if args.full_setup:
        ...
        print("\nTo test the API, run:")
        print(
            'curl -X POST "http://localhost:8000/api/v1/extract" -F "file=@path/to/your/document.pdf"'
        )
